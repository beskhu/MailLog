...
<head>
	<title>Api Fetching Mail - Déconnexion</title>
	...
	<link rel="icon" type="image/png" href="./media/favicon.png?mod=<?php echo filemtime(__DIR__.'/../media/favicon.png'); ?>"/>
				...
				<div style="text-align:center; color:#000;" id="lpslt_container" class="lpslt_resizable">
					...
					<?php
						...
						if (!isset($_SESSION['auth']) || !$_SESSION['auth']) {
							echo $_SESSION['locales']['nowLoggedout'][$_SESSION['current-language']].'
					<script type="text/javascript">setTimeout(function() { window.location=window.location.href.replace(/[a-z0-9-]+$/i, "index"); }, 3000);</script>';
						} else {
...
<?php
	if (isset($_SESSION['auth'], $_SESSION['passphrase']) && $_SESSION['auth']) {
                echo "\t",'<script type="text/javascript" class="lpslt_admin_script">
                onLoadedFunctions.push(function () { log.logged=true; });
        </script>',"\n";
    }
?>
</body>
