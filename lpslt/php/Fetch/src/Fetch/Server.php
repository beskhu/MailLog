...
class Server
{
     ...
     */
    public function __construct($serverPath, $port = 143, $service = 'imap', $flag = '')
    {
    ...
    protected function setImapStream()
    {
        if (!empty($this->imapStream)) {
            ...
            if ($imapStream === false)
                throw new \RuntimeException(implode(",", imap_errors()));
    ...
    public function getOrderedMessages($orderBy, $reverse, $limit)
    {
        ...
        if ($limit===-1) {
        	$slice=array_slice($msgIds, 0);
        } else {
        	$slice=array_slice($msgIds, 0, $limit);
        }
        $messages=[];
        for ($i=0; $i<count($slice); $i++) { 
        	$messages[$i]=$this->getMessageByUid($slice[$i]);
        }
        return $messages;
    }
    
    /**
     * Returns the emails in the current mailbox as an array of ImapMessage objects between date limits
     * ordered by some ordering
     *
     * @see    http://php.net/manual/en/function.imap-sort.php
     * @param  int       $orderBy
     ...
     * @return Message[]
     */
    public function getOrderedMessagesSinceDateBeforeDate($orderBy, $reverse, $limit, $sinceYmd, $beforeYmd)
    {
    	$months=["01"=>"Jan","02"=>"Feb","03"=>"Mar","04"=>"Apr","05"=>"May","06"=>"Jun","07"=>"Jul","08"=>"Aug","09"=>"Sep","10"=>"Oct","11"=>"Nov","12"=>"Dec"];
    	$criteria='SINCE '.(string)((int)substr($sinceYmd, 6, 2))."-".$months[substr($sinceYmd, 4, 2)]."-".substr($sinceYmd, 0, 4).' BEFORE '.(string)((int)substr($beforeYmd, 6, 2))."-".$months[substr($beforeYmd, 4, 2)]."-".substr($beforeYmd, 0, 4);
        $msgIds=imap_sort($this->getImapStream(), $orderBy, $reverse ? 1 : 0, SE_UID, $criteria);
        if (!$msgIds) {
        	$msgIds=[];
        }
        if ($limit===-1) {
